...
import { homePath, USER_ROLES, type AuthUser, type Organization } from '../lib/auth';
import { orgPlanPath } from '../lib/orgRoutes';
import { loginSchema, type LoginFormValues } from '../lib/schemas';
import { zodResolver } from '../lib/zodResolver';
import { setOrgSessionForUser } from '../lib/session';
...
export function Login() {
  ...
  const nextPath =
    nextRaw.startsWith('/') && !isSuperAdminPath(nextRaw) ? nextRaw : '';
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.user.role === USER_ROLES.SUPER_ADMIN) {
        toast.error('Use Visit Hub staff login for this account');
        return;
      }
      ...
      const needsPlan =
        (data.user.role === USER_ROLES.ORG_ADMIN || data.user.role === USER_ROLES.STAFF) &&
        data.hasActivePlan === false;
      if (needsPlan) {
        navigate(orgPlanPath(data.user.id), { replace: true });
        return;
      }
    ...
    <div className="app-bg relative min-h-screen overflow-hidden text-text">
      <main className="relative mx-auto flex w-full max-w-md flex-col px-4 pt-12 pb-16 sm:px-6 sm:pt-16">
        ...
        <div className="mb-8 text-center">
          ...
          {justRegistered ? (
            <p className="mt-2 text-sm text-mute">
              Your organisation admin account is ready. Sign in to open the dashboard.
            </p>
          ) : null}
        </div>
        ...
        <p className="mt-6 text-center text-sm text-mute">
          New here?{' '}
          <Link to="/create-account" className="font-semibold text-primary">
            Create account
          </Link>
        </p>
